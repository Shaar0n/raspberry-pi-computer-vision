...
# Initialize the camera with DEFAULT ISP (no overrides)
picam2 = Picamera2()
picam2.configure(
...
def process_frame(frame_bgr):
    ...
    resized_frame = cv2.resize(frame_bgr, (0, 0),
...
def draw_results(frame_bgr):
    ...
    for (top, right, bottom, left), name in zip(face_locations, face_names):
        ...
        cv2.rectangle(frame_bgr, (left, top), (right, bottom),
                      (255, 0, 0), 2)
        cv2.rectangle(
            frame_bgr, (left - 3, top - 35), (right + 3, top),
            (255, 0, 0), cv2.FILLED
        )
        font = cv2.FONT_HERSHEY_DUPLEX
        cv2.putText(
            frame_bgr, name, (left + 6, top - 6),
...
while True:
    ...
    # Correct conversion from XRGB8888 to BGR
